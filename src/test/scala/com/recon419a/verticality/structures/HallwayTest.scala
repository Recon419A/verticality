...
class HallwayTest extends FlatSpec with Matchers {
  ...
  it should "have a central hole in the 3x3 case" in {
    Hallway(3, 3, 2).voxel(1, 1, 0) shouldBe None
    Hallway(3, 3, 2).voxel(1, 1, 1) shouldBe None
  }

  it should "have a surrounding ring in the 3x3 case" in {
    Hallway(3, 3, 2).voxels should contain allOf (Voxel(0, 0, 0), Voxel(2, 2, 1), Voxel(2, 2, 0), Voxel(0, 1, 1))
    Hallway(3, 3, 2).voxels.size shouldBe 16
  }

  it should "scale in both dimensions" in {
    Hallway(4, 4, 2).voxels.size shouldBe 24
  }
}
